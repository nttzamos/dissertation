...
def update_student_name(student_id, new_student_name):
  ...
  cur.execute(query, (new_student_name, student_id))
...
def add_student_profiles(student_id, profiles):
  ...
  for profile in profiles:
    ...
    query = 'INSERT INTO student_profile VALUES (null, ?, ?)'
    cur.execute(query, (student_id, profile_id))
...
def remove_student_profiles(student_id, profiles):
  ...
  for profile in profiles:
    ...
    query = 'DELETE FROM student_profile WHERE student_id = ? AND profile_id = ?'
    cur.execute(query, (student_id, profile_id))
...
def get_student_profiles(student_id):
  ...
  query = ('SELECT name FROM profile INNER JOIN student_profile '
